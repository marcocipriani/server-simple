...
int setop(int cmd,int ownseq, int pktleft, void *arg){
...
int get(int iseq, void *pathname){
  ...
  setsock();
  npkt = check(setop(2, iseq, 0, pathname),"GET: setop"); /* ricevo da setop #pkt da ricevere */
...
int me;
int sockd, sockid; //per ora sockd = sockid
int nextseqnum;
struct sockaddr_in servaddr, cliaddr;
socklen_t len;
//pthread_mutex_t wsizemutex;
...
int get(int iseq, void *pathname){
  ...
  //controllo su buffer CLIENT
  if(freespacebuf(npkt)){
      sendack(sockd, iseq, ack->seq, 0, "ok");

      while(npkt>0)  {
        cargo = (struct pkt *)check_mem(malloc(sizeof(struct pkt *)), "GET-client:malloc cargo");
        check(recvfrom(sockd,cargo, MAXTRANSUNIT, 0, (struct sockaddr *)&servaddr, &len), "GET-client:recvfrom Cargo"); /*attesa di ricevere cargo dal server */
writeonbuff:
        //scriviiiii
        npkt--;
      }
    ...
    sendack(sockd, iseq, ack->seq, 0, "fullClientBuf"); //ack negativo
  }
}
...
transfer:
      ...
      printf("[Client #%d] Problem with connection to the Server \n", me);
      sendack(sockd, iseq, ack->seq, 0, "errConnToServ");
    }
}
...
printf("[Client #%d] Looking for list of default folder...\n", me);
            ...
            case 2: // get
                printf("Type filename to get and press ENTER: ");
                fscanf(stdin, "%s", arg);
                printf("[Client #%d] File selected is %s" , arg);
                if(get(seq, arg)){
                break;
put:
            case 3: // put
