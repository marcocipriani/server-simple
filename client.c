...
int get(int iseq, void *pathname){
  ...
  if(freespacebuf(npkt)){
      ack = (struct pkt *)check_mem(malloc(sizeof(struct pkt *)), "GET-client:malloc ack");
      sendack(sockd, ack->seq, 0, "ok");
...
writeonbuff:
    ...
    ack = (struct pkt *)check_mem(malloc(sizeof(struct pkt *)), "GET-client:malloc ack");
    sendack(sockd, ack->seq, 0, "Client non può ricevere"); //ack negativo
  }
